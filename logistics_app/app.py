...
# ─── CABECERA ─────────────────────────────────────────────────────────────────
# Logos y nombre del apartado activo
_sec_cfg       = next((s for s in APP_SECTIONS if s["id"] == _seccion_activa), {})
_sec_nombre    = _sec_cfg.get("nombre", "")
_sec_logos     = _sec_cfg.get("logos", [])
_sec_logos_html = ""
for _lf in _sec_logos:
    _lb = _logo_file_b64(_lf)
    if _lb:
        _sec_logos_html += (
...
<div style="margin-left:auto;text-align:right;">
  ...
  <span style="color:rgba(255,255,255,0.5);font-size:0.72rem;letter-spacing:1.5px;
    text-transform:uppercase;font-weight:600;">{_sec_nombre}</span>
</div>
"""
...
with tabs[0]:
    ...
    # Resumen por almacén: tabla con cajas y unidades por producto
    st.markdown("### Resumen por almacén")
    _prods_cat = _datos.get("productos", {})
    _rows_resumen = []
    for _alm_name in _df_num.index:
        _fila = _df_num.loc[_alm_name]
        for _prod in _prod_cols:
            _cajas = int(float(_fila.get(_prod, 0) or 0))
            if _cajas == 0:
                continue
            ...
            _rows_resumen.append({
                ...
                "Producto":  _prod,
                "Cajas":     _cajas,
                "Unidades":  _cajas * _uds_x_caja,
            })
    if _rows_resumen:
        _df_resumen = pd.DataFrame(_rows_resumen)
        st.dataframe(_df_resumen, use_container_width=True, hide_index=True, height=400)
    else:
